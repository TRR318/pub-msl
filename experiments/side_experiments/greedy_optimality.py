...
round_ = 3
dataset_names = {"thorax": "Coronary Heart Disease", 42900: "Breast Cancer Coimbra"}
for dataset, scoreset in [["thorax", [0, 1, 2, 3]], [42900,[0, 1, 2, -1, -2]]]:
    ...
    for scores, result in tqdm(
        ...
        id_ = np.count_nonzero(scores), round(result, round_)
    ...
    cascade = [(i, round(clf.score(X, y), round_)) for i, clf in enumerate(psl)]
    ...
    fig.savefig(f"fig/{dataset}_greedy_search.png", dpi=200, bbox_inches="tight")
