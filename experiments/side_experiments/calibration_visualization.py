...
for variant in ["isotonic", "beta"]:
    psl = ProbabilisticScoringList(score_set, stage_clf_params=dict(calibration_method=variant))
    psl.fit(X, y)
...
offset = np.array([np.tile(np.array(sorted(np.arange(per_line)-per_line/2, key=abs)), lines)*.07,
...
ax.step(calibrators["isotonic"].X_thresholds_, calibrators["isotonic"].y_thresholds_, label="Isotonic Regression")
